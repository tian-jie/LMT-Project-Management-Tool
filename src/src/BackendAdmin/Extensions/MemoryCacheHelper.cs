...
public class MemoryCacheHelper
{
    ...
    private static MemoryCacheHelper _memoryCacheHelper = new MemoryCacheHelper();
    ...
    /// <param name="expiresSliding">滑动过期时长（如果在过期时间内有操作，则以当前时间点延长过期时间）</param>
    /// <param name="expiressAbsoulte">绝对过期时长</param>
    /// <param name="factory">取缓存数据的函数</param>
    /// <returns></returns>
    public async Task<List<T>> GetOrCreateAsync<T>(string key, Func<ICacheEntry, Task<List<T>>> factory)
    {
        if (key == null)
            throw new ArgumentNullException(nameof(key));
